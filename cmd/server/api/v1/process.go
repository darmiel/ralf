...
// ProcessRoutes holds the dependencies for the /process routes.
type ProcessRoutes struct {
	cacheService service.CacheService
}

// NewProcessRoutes creates a new ProcessRoutes with the necessary dependencies.
func NewProcessRoutes(cacheService service.CacheService) *ProcessRoutes {
...
func (p *ProcessRoutes) processGetHandler(c *fiber.Ctx) error {
	q := c.Query("template")
	if q == "" {
...
func (p *ProcessRoutes) processPostHandler(c *fiber.Ctx) error {
	return p.processContent(c.Body(), c)
}

// processContent processes the content and returns the resulting calendar.
func (p *ProcessRoutes) processContent(content []byte, ctx *fiber.Ctx) error {
...
func (p *ProcessRoutes) executeFlow(
	ctx context.Context,
...
func (p *ProcessRoutes) getSource(
	ctx context.Context,
	source model.Source,
...
func (p *ProcessRoutes) getSourceWithRequest(
	ctx context.Context,
	source model.Source,
	cacheDuration time.Duration,
	cacheKey string,
